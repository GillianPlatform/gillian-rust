...
pub fn compile_logic<'tcx, 'genv>(
    ...
    if is_abstract_predicate(did, tcx) {
        ...
        let lemma = lemma::LemmaCtx::new(global_env, did, temp_gen, is_lemma(did, tcx)).compile();
        vec![LogicItem::Lemma(lemma)]
        // Has to b safe, because we know there is exactly one definition
    } else if is_extract_lemma(did, tcx) {
        extract_lemma::ExtractLemmaCtx::new(global_env, did, temp_gen, parent_trusted(did, tcx))
            .compile()
