...
use rustc_middle::ty::{AliasTy, ParamEnv};
...
pub trait TraitSolver<'tcx> {
    ...
    #[deprecated = "This function is unsound. Use the explicit `resolve_candidate` instead"]
    ...
    #[deprecated = "This function is unsound."]
...
impl<'tcx, T: HasTyCtxt<'tcx>> TraitSolver<'tcx> for T {
    fn resolve_candidate(&self, def_id: DefId, substs: GenericArgsRef<'tcx>) -> ResolvedImpl<'tcx> {
        resolve_candidate(self.tcx(), self.tcx().param_env(def_id), def_id, substs)
    }
