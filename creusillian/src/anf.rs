...
pub(crate) enum CoreTerm {
    ...
    Forall(Vec<VarKind>, Box<CoreTerm>),
}
...
impl ToTokens for PreGil {
    fn to_tokens(&self, tokens: &mut TokenStream) {
        match self {
            ...
            PreGil::Forall(var, body) => tokens.extend(quote! {
                (forall < #var : _ > #body )
            }),
...
pub(crate) fn print_gilsonite(t: CoreTerm) -> syn::Result<TokenStream> {
    match t {
        ...
        CoreTerm::Forall(ids, t) => {
            let t = print_gilsonite(*t)?;
            Ok(quote! { forall < #(#ids),* > #t})
        }
...
fn core_conjunct_to_sl(t: CoreTerm) -> PreGil {
    match t {
        ...
        CoreTerm::Forall(v, b) => v.into_iter().fold(core_conjunct_to_sl(*b), |acc, v| {
            PreGil::Forall(v, Box::new(acc))
        }),
    }
}
...
pub(crate) fn elim_match_form(t: &mut CoreTerm) {
    match t {
        ...
        CoreTerm::Forall(_, e) => elim_match_form(e),
        CoreTerm::Var(_) => (),
        CoreTerm::Lit(_) => (),
        CoreTerm::Path(_) => (),
    }
