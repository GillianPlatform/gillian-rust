...
module TreeBlock = struct
      ...
      if Ty.is_array_of ~array_ty:expected_ty ~inner_ty:ty then
        ...
        if%ent Ty.sem_equal expected_ty ty then
          ...
          | Enum { discr; fields } ->
              let ptvs =
                let tys =
                  List.to_seq (Ty.variant_fields ~idx:discr ty)
                in
      ...
      | Array l
        when Ty.is_array_of ~array_ty:node_ty ~inner_ty:expected_inner_ty ->
          let++ l, lk =
            all ~lk
              (fun ~lk x ->
                to_rust_maybe_uninit ~loc ~proj ~ty:expected_inner_ty ~lk
                  x)
              [] l
          in
          (Expr.EList l, lk)
  ...
  let of_rust_many_maybe_uninit ~ty (e : Expr.t) : (t, Err.t) DR.t =
    ...
    | Expr.EList l, Ty.Array { ty = inner_ty; _ } ->
        let++ elements =
          DR_list.map (of_rust_maybe_uninit ~ty:inner_ty) l
        in
  ...
  *)
    ...
    if%sat Range.is_equal range lc.range then (
      ...
      | Some structural, None ->
          ...
          let* new_node, lk =
            ...
            extract_range_structural ~lk ~index_ty:lc.index_ty
              ~range:rel_range structural
          in
          let** (value, new_tree), lk =
            extract_and_apply ~lk ~return_and_update ~index_ty
  ...
  and extract_and_apply ~return_and_update ~range ~index_ty t ~lk =
    ...
    if%sat eq then return_and_update ~lk t
      ...
      | Structural s ->
          ...
          extract_and_apply ~lk ~return_and_update ~range ~index_ty
            new_node
      | Laid_out_root lc ->
  ...
  let frame_path
    ...
    | Ty.Array { length; ty } ->
        ...
        if%sat Formula.Infix.(length #== Expr.zero_i) then
          ...
          frame_slice ~can_extend:true ~lk ~return_and_update ~ty t path
            length
    | _ ->
  ...
  let frame_proj
    ...
    Logging.verbose (fun m ->
        m "After path reduction: %a" Projections.pp_path path);
    let++ (ret, root), lk =
      frame_path ~lk ~return_and_update ~ty t path
    in
    ((ret, { outer with root }), lk)
  ...
  let copy_nonoverlapping
    ...
    if%sat Formula.Infix.(size #== Expr.zero_i) then DR.ok (to_block, lk)
      ...
      let++ ((), mem), lk =
        frame_slice_outer ~lk ~return_and_update ~ty to_block to_proj
          size
      in
      (mem, lk)
  ...
  let prod_proj ~lk t proj ty value =
    let* new_block =
      ...
      in
      let++ (_, new_block), lk =
        frame_proj ~ty ~lk ~return_and_update t proj
      in
      (new_block, lk)
    in
  ...
  let prod_uninit ~loc:_ t proj ty ~lk =
    ...
    let* result =
      ...
      in
      let++ (_, new_block), lk =
        frame_proj ~ty ~return_and_update ~lk t proj
      in
      (new_block, lk)
    in
    match result with
  ...
  let prod_maybe_uninit ~loc:_ ~lk t proj ty maybe_value =
    let* result =
      ...
      in
      let++ (_, new_block), lk =
        frame_proj ~lk ~ty ~return_and_update t proj
      in
      (new_block, lk)
    in
    match result with
...
let store ~lk (mem : t) loc proj ty value =
  if%sat TypePreds.empty_array ty then DR.ok (mem, lk)
    ...
    let** block = find_not_freed loc mem in
    let++ new_block, lk =
      TreeBlock.store_proj ~loc ~lk block proj ty value
    in
...
let copy_nonoverlapping
  ...
  if String.equal from_loc to_loc then
    ...
    if%sat Formula.Infix.(size #== Expr.zero_i) then DR.ok (mem, lk)
      ...
      let++ new_to_block, lk =
        TreeBlock.copy_nonoverlapping ~lk from_block from_proj to_block
          to_proj ty size
      in
...
let cons_value ~lk (mem : t) loc proj ty =
  if%sat TypePreds.empty_array ty then
    ...
    let** block = find_not_freed loc mem in
    let++ (value, outer), lk =
      TreeBlock.cons_proj ~loc ~lk block proj ty
    in
    let new_heap =
...
let cons_uninit ~lk (mem : t) loc proj ty =
  ...
  if%sat TypePreds.empty_array ty then DR.ok (mem, lk)
    ...
    let** block = find_not_freed loc mem in
    let++ ((), outer), lk =
      TreeBlock.cons_uninit ~loc ~lk block proj ty
    in
    let new_heap =
      if TreeBlock.outer_is_empty outer then MemMap.remove loc mem
      else MemMap.add loc (T outer) mem
...
let prod_many_maybe_uninits
  ...
  if%sat Formula.Infix.(size #== Expr.zero_i) then
    ...
    in
    let+ new_block, lk =
      TreeBlock.prod_many_maybe_uninits ~loc ~lk root proj ty size
        maybe_values
    in
    (MemMap.add loc (T new_block) mem, lk)
