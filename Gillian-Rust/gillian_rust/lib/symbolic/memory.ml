...
let clear t = { t with heap = Heap.empty }
...
let execute_store mem args =
  ...
  | [ loc; proj; ty; value ] ->
      ...
      let++ new_heap = Heap.store ~tyenv ~lfts heap loc proj ty value in
...
let execute_load mem args =
  ...
  | [ loc; proj; ty; Expr.Lit (Bool copy) ] ->
      ...
      let* proj = projections_of_expr proj in
      let** loc = resolve_loc_result loc in
      let++ value, new_heap = Heap.load ~tyenv ~lfts heap loc proj ty copy in
...
let execute_load_discr mem args =
  ...
  | [ loc; proj; enum_typ ] ->
      ...
      let* proj = projections_of_expr proj in
      let** loc = resolve_loc_result loc in
      let++ discr = Heap.load_discr ~tyenv ~lfts heap loc proj enum_typ in
...
let execute_free mem args =
  ...
  | [ loc; proj; ty ] ->
      ...
      let** loc = resolve_loc_result loc in
      let ty = Ty.of_expr ty in
      let++ new_heap = Heap.free ~lfts ~tyenv heap loc ty in
      make_branch ~mem:{ mem with heap = new_heap } ()
...
let execute_new_lifetime mem args =
  ...
  | [ lft ] ->
      ...
      let lfts = Lft_ctx.set_alive mem.lfts lft in
      DR.ok (make_branch ~mem:{ mem with lfts } ())
  | _ -> Fmt.failwith "Invalid arguments for new_lft"

let execute_end_lifetime mem args =
  let open Syntaxes.Result in
  match args with
  | [ lft ] ->
      let lft = Lft.of_expr lft in
      Lft_ctx.kill mem.lfts lft
      |> Result.map (fun lfts -> make_branch ~mem:{ mem with lfts } ())
      |> Delayed.return
  | _ -> Fmt.failwith "Invalid arguments for end_lft"

let execute_get_alive_lft mem args =
  let open Syntaxes.Result in
  match args with
  | [ lft ] ->
      let res =
        let+ () = Lft_ctx.check_alive mem.lfts lft in
        make_branch ~mem ()
      in
      Delayed.return res
  | _ -> Fmt.failwith "Invalid arguments for get_alive_lft"

let execute_set_alive_lft = execute_new_lifetime

let execute_rem_alive_lft mem args =
  match args with
  | [ lft ] ->
...
let substitution_in_place s mem =
  Delayed.return
  @@ { mem with heap = Heap.substitution ~tyenv:mem.tyenv mem.heap s }
